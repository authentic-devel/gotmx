...
func ExampleEngine_Render() {
	engine, _ := gotmx.New()
	_ = engine.LoadHTML(`<div data-g-define="greeting" data-g-inner-text="[[ .Name ]]">placeholder</div>`)

	_ = engine.Render(context.Background(), os.Stdout, "greeting", map[string]any{"Name": "World"})
	// Output: <div>World</div>
}

func ExampleEngine_RenderString() {
	engine, _ := gotmx.New()
	_ = engine.LoadHTML(`<span data-g-define="badge" data-g-inner-text="[[ .Label ]]">x</span>`)
...
func ExampleEngine_Render_iteration() {
	engine, _ := gotmx.New()
	_ = engine.LoadHTML(`<ul data-g-define="list"><li data-g-outer-repeat="[[ .Items ]]" data-g-inner-text="[[ . ]]">placeholder</li></ul>`)

	data := map[string]any{
		"Items": []string{"Alice", "Bob", "Carol"},
	}
	_ = engine.Render(context.Background(), os.Stdout, "list", data)
	// Output: <ul><li>Alice</li><li>Bob</li><li>Carol</li></ul>
}

func ExampleEngine_Render_conditional() {
	engine, _ := gotmx.New()
	_ = engine.LoadHTML(`<div data-g-define="status"><span data-g-if="[[ .Active ]]">Active</span><span data-g-if="[[ .Inactive ]]">Inactive</span></div>`)

	_ = engine.Render(context.Background(), os.Stdout, "status", map[string]any{"Active": true, "Inactive": false})
...
func ExampleEngine_Render_composition() {
	...
	_ = engine.Render(context.Background(), os.Stdout, "page", nil)
	// Output: <div><button class="btn"><span>Submit</span></button></div>
}

func ExampleEngine_RenderString_withSlots() {
	engine, _ := gotmx.New()
	_ = engine.LoadHTML(`<div data-g-define="card"><header data-g-define-slot="header">Default</header><main data-g-define-slot="">Body</main></div>`)
...
func ExampleEngine_Render_escaping() {
	engine, _ := gotmx.New()
	_ = engine.LoadHTML(`<div data-g-define="safe" data-g-inner-text="[[ .Input ]]">placeholder</div>`)

	// g-inner-text always escapes HTML, protecting against XSS
	_ = engine.Render(context.Background(), os.Stdout, "safe", map[string]any{
...
func ExampleUnescaped() {
	engine, _ := gotmx.New()
	_ = engine.LoadHTML(`<div data-g-define="raw" data-g-inner-html="[[ .Html ]]">placeholder</div>`)

	// Use g-inner-html for trusted HTML content
	_ = engine.Render(context.Background(), os.Stdout, "raw", map[string]any{
